...
libname lib '~/BIOS669/Data/Macro' access=readonly;
...
proc sql noprint;
    select count(*) into :overall trimmed from rptc.rptc; 
    select count(*) into :metN    trimmed from rptc.rptc where trt='A';
    select count(*) into :placebo trimmed from rptc.rptc where trt='B';
quit;
